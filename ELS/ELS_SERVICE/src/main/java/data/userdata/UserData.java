...
public class UserData extends UnicastRemoteObject implements UserDataService {
	...
	public int deleteUser(String userID) throws RemoteException {
		...
		/*
		 * for (int i = 0; i < userList.size(); i++) { UserPO tempUserPO =
		 * (UserPO) (userList.get(i));
		 * 
		 * if (tempUserPO.getUserID().equals(userID)) {
		 * 
		 * if (!tempUserPO.getOrganization().equals("")) { String
		 * newOrganization = tempUserPO.getOrganization(); switch
		 * (tempUserPO.getProfession()) { // 快递员 case courier: File file =
		 * FileGetter.getFile("expressInfo/" + newOrganization +
		 * "-express.dat"); try { ArrayList<ExpressPO> expressPOs = null; if
		 * (file.exists()) { ObjectInputStream in = new ObjectInputStream( new
		 * FileInputStream(file)); expressPOs = (ArrayList<ExpressPO>) in
		 * .readObject(); in.close();
		 * 
		 * int size = expressPOs.size(); for (int j = 0; j < size; j++) { if
		 * (expressPOs.get(j).getID() .equals(tempUserPO.getUserID())) {
		 * expressPOs.remove(j); break; } }
		 * 
		 * ObjectOutputStream out = new ObjectOutputStream( new
		 * FileOutputStream(file)); out.writeObject(expressPOs); out.close(); }
		 * 
		 * } catch (Exception e) { e.printStackTrace(); }
		 * 
		 * break; case businessHallCounterman:
		 * 
		 * OrganizationPO organizationPO1 = (new OrganizationData())
		 * .findOrganizationByID(newOrganization); BusinessPO po1 = new
		 * BusinessPO(tempUserPO.getName(), tempUserPO.getUserID(), "0",
		 * organizationPO1);
		 * 
		 * File file1 = FileGetter.getFile("businessInfo/" + newOrganization +
		 * "-business.dat");
		 * 
		 * try { ArrayList<BusinessPO> businessPOs = null; if (file1.exists()) {
		 * ObjectInputStream in = new ObjectInputStream( new
		 * FileInputStream(file1)); businessPOs = (ArrayList<BusinessPO>) in
		 * .readObject(); in.close();
		 * 
		 * int size = businessPOs.size(); for (int j = 0; j < size; j++) { if
		 * (businessPOs.get(j).getID() .equals(tempUserPO.getUserID())) {
		 * businessPOs.remove(j); break; } }
		 * 
		 * ObjectOutputStream out = new ObjectOutputStream( new
		 * FileOutputStream(file1)); out.writeObject(businessPOs); out.close();
		 * }
		 * 
		 * } catch (Exception e) { e.printStackTrace(); }
		 * 
		 * break; case intermediateCenterCounterman:
		 * 
		 * OrganizationPO organizationPO2 = (new OrganizationData())
		 * .findOrganizationByID(newOrganization); IntermediatePO po2 = new
		 * IntermediatePO( organizationPO2, tempUserPO.getName(),
		 ...
		 * 
		 * try { ArrayList<IntermediatePO> intermediatePOs = null; if
		 * (file2.exists()) { ObjectInputStream in = new ObjectInputStream( new
		 * FileInputStream(file2)); intermediatePOs =
		 * (ArrayList<IntermediatePO>) in .readObject(); in.close();
		 * 
		 * int size = intermediatePOs.size(); for (int j = 0; j < size; j++) {
		 * if (intermediatePOs.get(j).getID() .equals(tempUserPO.getUserID())) {
		 * intermediatePOs.remove(j); break; } }
		 * 
		 * ObjectOutputStream out = new ObjectOutputStream( new
		 * FileOutputStream(file2)); out.writeObject(intermediatePOs);
		 * out.close(); }
		 * 
		 * } catch (Exception e) { e.printStackTrace(); }
		 * 
	...
	public int modifyUserOrganization(String userID, String newOrganizationID)
		...
		for (int i = 0; i < userList.size(); i++) {
			...
			if (tempUserPO.getUserID().equals(userID)) {
				...
				// 快递员
				case courier:
					ExpressPO po1 = new ExpressPO(tempUserPO.getName(),
							...
							"0", new ArrayList<String>(), organizationPO,
				...
				case businessHallCounterman:
					BusinessPO po2 = new BusinessPO(tempUserPO.getName(),
							tempUserPO.getUserID(), tempUserPO.getPassword(),
							tempUserPO.getProfession(),
							tempUserPO.getOrganization(),
							tempUserPO.getSalaryPlan(),
	...
	public String getUserIDPost(ProfessionType profession)
		...
		for (int i = 0; i < userList.size(); i++) {
			...
			if (tempUserPO.getProfession().equals(profession)) {
				int tempPost = Integer.parseInt(tempUserPO.getUserID().split(
						"-")[1]);
...
//						AuthorityType.administrator, 0));
				 ...
				 * SalaryPlanType.basicStaffSalaryPlan, AuthorityType.lowest,
				 * 0)); userData.addUser(new UserPO("张方浩", "ZZZX-00002",
				 * "123456", ProfessionType.intermediateCenterCounterman,
				 * "021-0", SalaryPlanType.basicStaffSalaryPlan,
				 * AuthorityType.lowest, 0)); userData.addUser(new UserPO("张海涛",
				 * "ZZZX-00003", "123456",
				 * ProfessionType.intermediateCenterCounterman, "010-0",
				 * SalaryPlanType.basicStaffSalaryPlan, AuthorityType.lowest,
				 * 0)); userData.addUser(new UserPO("张晨剑", "ZZZX-00004",
				 * "123456", ProfessionType.intermediateCenterCounterman,
				 * "020-0", SalaryPlanType.basicStaffSalaryPlan,
				 * AuthorityType.lowest, 0));
				 * 
				 * userData.addUser(new UserPO("万云天", "CK-00001", "123456",
				 * ProfessionType.stockman, "025-0-CK",
				 * SalaryPlanType.basicStaffSalaryPlan, AuthorityType.lowest,
				 * 0)); userData.addUser(new UserPO("汪盼", "CK-00002", "123456",
				 * ProfessionType.stockman, "021-0-CK",
				 * SalaryPlanType.basicStaffSalaryPlan, AuthorityType.lowest,
				 * 0)); userData.addUser(new UserPO("王成垚", "CK-00003", "123456",
				 * ProfessionType.stockman, "010-0-CK",
				 * SalaryPlanType.basicStaffSalaryPlan, AuthorityType.lowest,
				 * 0)); userData.addUser(new UserPO("王栋", "CK-00004", "123456",
				 ...
				 * SalaryPlanType.basicStaffSalaryPlan, AuthorityType.lowest,
				 * 0)); userData.addUser(new UserPO("杨华安", "YYT-00002",
				 * "123456", ProfessionType.businessHallCounterman, "025001",
				 * SalaryPlanType.basicStaffSalaryPlan, AuthorityType.lowest,
				 * 0)); userData.addUser(new UserPO("杨三洋", "YYT-00003",
				 * "123456", ProfessionType.businessHallCounterman, "021000",
				 * SalaryPlanType.basicStaffSalaryPlan, AuthorityType.lowest,
				 * 0)); userData.addUser(new UserPO("杨雁飞", "YYT-00004",
				 * "123456", ProfessionType.businessHallCounterman, "021001",
				 * SalaryPlanType.basicStaffSalaryPlan, AuthorityType.lowest,
				 * 0)); userData.addUser(new UserPO("曾攀", "YYT-00005", "123456",
				 * ProfessionType.businessHallCounterman, "010000",
				 * SalaryPlanType.basicStaffSalaryPlan, AuthorityType.lowest,
				 * 0)); userData.addUser(new UserPO("朱文沛", "YYT-00006",
				 * "123456", ProfessionType.businessHallCounterman, "010001",
				 * SalaryPlanType.basicStaffSalaryPlan, AuthorityType.lowest,
				 * 0)); userData.addUser(new UserPO("朱宇翔", "YYT-00007",
				 * "123456", ProfessionType.businessHallCounterman, "020000",
				 * SalaryPlanType.basicStaffSalaryPlan, AuthorityType.lowest,
				 * 0)); userData.addUser(new UserPO("邹瀚真", "YYT-00008",
				 * "123456", ProfessionType.businessHallCounterman, "020001",
				 * SalaryPlanType.basicStaffSalaryPlan, AuthorityType.lowest,
				 * 0));
				 * 
				 * userData.addUser(new UserPO("张词校", "KD-00001", "123456",
				 * ProfessionType.courier, "025000",
				 * SalaryPlanType.courierSalaryPlan, AuthorityType.lowest, 50));
				...
				/*
				 * String postNow =
				 * userData.getUserIDPost(ProfessionType.courier);
				 * System.out.println(postNow);
				 * 
				 * UserPO userpo1 = userData.findUserByID("KD-00001"); if
				 * (userpo1 != null) System.out.println("Find the user: " +
				 * userpo1.getName() + " " + userpo1.getUserID() + " " +
				 * userpo1.getOrganization() + " " + userpo1.getProfession());
				 * else System.out.println("Cannot find the user");
				 * 
				 * userData.modifyUserOrganization("张Doge", "025000");
				 * System.out.println("修改机构后:"); ArrayList<UserPO> userpoList3 =
				 * userData.showAllUsers(); if (userpoList3 != null) { for (int
				 * i = 0; i < userpoList3.size(); i++) { UserPO tempUserpo =
				 * userpoList3.get(i); System.out.println(tempUserpo.getName() +
				 * "  " + tempUserpo.getUserID() + "  " +
				 * tempUserpo.getOrganization() + "  " +
				 ...
				 * i = 0; i < userpoList1.size(); i++) { UserPO tempUserpo =
				 * userpoList1.get(i); System.out.println(tempUserpo.getName() +
				 * "  " + tempUserpo.getUserID() + "  " +
				 * tempUserpo.getOrganization() + "  " +
				 * tempUserpo.getProfession()); } } else System.out.println(
				 * "Cannot find the user");
				 * 
				 * // userData.deleteUser("CW-00001");
				 * System.out.println("删除后:"); ArrayList<UserPO> userpoList2 =
				 * userData.showAllUsers(); if (userpoList2 != null) { for (int
				 * i = 0; i < userpoList2.size(); i++) { UserPO tempUserpo =
